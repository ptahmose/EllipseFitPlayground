...
public:
	...
	static EllipseAlgebraicParameters CreateFrom5Points(tFloat p1x, tFloat p1y, tFloat p2x, tFloat p2x, tFloat p3x, tFloat p3y, tFloat p4x, tFloat p4y, tFloat p5x, tFloat p5y)
	{
		...
		return EllipseAlgebraicParameters::FromPoints(pp0, pp1, pp2, pp3, pp4, pp5);
	}
...
public:
	...
	static EllipseParameters<tFloat> Invalid = { std::numeric_limits<tFloat>::quiet_NaN(),std::numeric_limits<tFloat>::quiet_NaN(), std::numeric_limits<tFloat>::quiet_NaN(), std::numeric_limits<tFloat>::quiet_NaN(), std::numeric_limits<tFloat>::quiet_NaN() };

	static EllipseParameters<tFloat> FromAlgebraicParameters(const EllipseAlgebraicParameters& algebraic)
	{
		EllipseParameters<tFloat> p;
		tFloat s = 4 * algebraic.a*algebraic.c - algebraic.b*algebraic.b;
		tFloat sigma = 4 * (algebraic.c*algebraic.d*algebraic.d + algebraic.a*algebraic.e*algebraic.e - algebraic.b*algebraic.d*algebraic.e - algebraic.f*(4 * algebraic.a*p1.c - algebraic.b*algebraic.b)) / (s*s);
		...
		s *= s;
		double v1 = sigma*algebraic.a + sigma*algebraic.c + sqrt(s + squared(sigma*algebraic.b));
		double v2 = sigma*algebraic.a + sigma*algebraic.c - sqrt(s + squared(sigma*algebraic.b));

		p.a = sqrt(abs((sigma*algebraic.a + sigma*algebraic.c + sqrt(s + sigma*algebraic.b*sigma*algebraic.b)) / 2));
		p.b = sqrt(abs((sigma*algebraic.a + sigma*algebraic.c - sqrt(s + sigma*algebraic.b*sigma*algebraic.b)) / 2));
